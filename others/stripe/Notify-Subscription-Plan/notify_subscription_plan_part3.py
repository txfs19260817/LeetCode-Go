class Part3Notifier:
    """
    Part 3:
    - Includes Part 1 and Part 2 behavior
    - subscription_changes may include extension
    - On extension day, send one extra [Renewed]
    - Upcoming/expired emails use final end date after all extensions
    """
    ...
    def _group_changes_by_user(self):
        ...
        for idx, change in enumerate(self.subscription_changes):
            ...
            # Keep both new_plan and extension in one normalized structure.
            grouped.setdefault(user_name, []).append(
                {
                    "idx": idx,
                    "day": int(day),
                    "new_plan": change.get("new_plan"),
                    "extension": int(change.get("extension", 0)),
                }
            )
    ...
    def _extract_plan_changes(self, user_changes):
        # Plan switch timeline; extension does not change plan by itself.
    ...
    def _plan_on_day(self, initial_plan, plan_changes, day):
        # Last plan change on/before `day` wins.
    ...
    def _final_end_date(self, begin_date, duration, user_changes):
        # Part 3: all extensions shift the final expiry.
        end_date = begin_date + duration
        for change in user_changes:
            end_date += change["extension"]
        return end_date
    ...
    def generate_notifications(self):
        ...
        for user in self.user_accounts:
            ...
            end_date = self._final_end_date(begin_date, duration, user_changes)

            for change in user_changes:
                # Part 2 behavior preserved in Part 3.
                if change["new_plan"] is not None:
                    current_plan = self._plan_on_day(
                        initial_plan, plan_changes, change["day"]
                    )
                    records.append(
                        (
                            change["day"],
                            f"[Updated] {name}, subscribe in plan {current_plan}",
                        )
                    )

                # Part 3 extra email for extension on that day.
                if change["extension"] > 0:
                    current_plan = self._plan_on_day(
                        initial_plan, plan_changes, change["day"]
                    )
                    records.append(
                        (
                            change["day"],
                            f"[Renewed] {name}, subscribe in plan {current_plan}",
                        )
                    )

            # Regular schedule emails are computed from final end_date (after extensions).
...
def generate_notifications(input_data):
    return Part3Notifier(input_data).generate_notifications()
