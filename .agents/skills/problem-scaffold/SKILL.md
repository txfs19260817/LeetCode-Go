...
description: Create a coding-problem folder scaffold with README, Go source, Go tests, and a Python solution with __main__ asserts. Use when the user provides a problem statement and asks to create a new problem folder under a target path, either as a full implementation or a skeleton.
---
...
- In skeleton mode, include TODO stubs and return zero values or empty structs while preserving the required signature.
...
- In skeleton mode, skip tests that require unimplemented logic.
...
- In skeleton mode, keep `__main__` asserts minimal and avoid false failures.
